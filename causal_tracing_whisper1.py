...
def forward_with_encoder_prefix(model, tokenizer: LlamaTokenizer, prompt, device, target, prefix_embedding, repeat, encoder_input_text):
    encoder_output = from_prefix(model, tokenizer, device, prefix_embedding, encoder_input_text).last_hidden_state

    decoder_input = torch.tensor([[model.config.decoder_start_token_id] + tokenizer.encode(prompt, add_special_tokens=False)],
                                 device=device)
...
def forward_with_prefix(model, tokenizer: LlamaTokenizer, prompt, device, target, prefix_embedding, repeat,
    ...
    decoder_input = torch.tensor([[model.config.decoder_start_token_id] + (
                [0] * (prefix_embedding.shape[0]-1 if prefix_embedding.shape[0]-1 > 0 else 0)) + tokenizer.encode(prompt, add_special_tokens=False)],
                                 device=device)
...
def fine_tuning(model: LlamaForCausalLM, tokenizer: LlamaTokenizer, prompt, device, model_forwarder, objects,
    ...
    for num_steps in range(max_steps):
        with torch.enable_grad():
            ...
            decoder_input = torch.tensor([[model.config.decoder_start_token_id] + tokenizer.encode(
                decoder_input_text, add_special_tokens=False)], device=device)
            labels = torch.cat(
                [torch.full_like(decoder_input, -100)[:, 1:], torch.tensor([[target]], device=device)],
                -1)
            output_dict = model.forward(encoder_outputs=(encoder_outputs,),
                                        decoder_input_ids=decoder_input, labels=labels,
                                        return_dict=True)
...
def prefix_tuning(model: LlamaForCausalLM, tokenizer: LlamaTokenizer, prompt, device, model_forwarder, objects,
    ...
    prefix_embedding = torch.stack([prefix_embedding] * len_prefix, 0) if len_prefix > 0 else prefix_embedding.unsqueeze(0)[:0]
...
class ModelForwarderEncDec:
    ...
    def forward(self, model: T5ForConditionalGeneration, tokenizer: T5Tokenizer, prompt, device, repeat, obj):
        ...
        with torch.no_grad():
            ...
            decoder_input = torch.tensor([[model.config.decoder_start_token_id]+tokenizer.encode(decoder_input_text, add_special_tokens=False)], device=device)
        ...
        with torch.no_grad():
            if self.cached_encoder_input[0] == encoder_input_text:
                ...
                if self.adaptor is None:
                    ...
                    encoder_outputs = from_lora(model, tokenizer, device, self.adaptor, encoder_input_text).last_hidden_state
    ...
    if not skip_creation or not os.path.exists(partial_path):
        with ResumeAndSaveFactDataset(partial_path, 10) as partial_dataset:
            for entry in tqdm(fakepedia, desc="Filtering facts"):
                ...
                if args.max_steps > 0:
                    adaptor, num_steps = fine_tuning(model, tokenizer, model_forwarder.edit_prompt(prompt)[1] if hasattr(model_forwarder, "edit_prompt") else prompt, device, model_forwarder, target_tokens,
                                                       max_steps=args.max_steps, encoder_input_text=model_forwarder.edit_prompt(prompt)[0] if hasattr(model_forwarder, "edit_prompt") else None)
        ...
        with ResumeAndSaveFactDataset(causal_traces_path, save_interval=1) as dataset:
            for entry in (pbar := tqdm(facts, desc=f"Running causal tracing on {bucket} facts")):
                ...
                output_entry = process_entry(causal_tracer, prompt, fact.get_subject(), fact.get_object(), target_token,
                                             bucket,
                                             adaptor if adaptor is None else torch.tensor(adaptor, device=device), args,
                                             pbar)
