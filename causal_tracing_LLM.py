...
Original file is located at
    https://colab.research.google.com/drive/1iIHMyoeRIpcivj3faW6fGV6WtjIHFqYy
...
def process_entry(causal_tracer: MaskedCausalTracer, prompt: str, subject: str, obj: str, target_token: str,
    ...
    for token_i in (list(range(0, min(31, subject_tokens_range[0]), 10)) +
                    list(range(subject_tokens_range[0], num_tokens))):
        d = {}
...
class Namespace1:
    def __init__(self):
        self.token = "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"
...
class Namespace2:
    def __init__(self):
        ...
        self.n_samples_per_label = 1781 if False else 20
        ...
        for kind in ["hidden", "mlp", "attn"]:
            ...
            for token in tokens:
                if len(token[kind]) > 1:
                    ...
                    interval_explained_average = max(token[kind][target_token]["probs"] - corrupted_score, 0)
...
def group_results2(facts_grounded, facts_unfaithful, args):
    ...
    process_facts2("grounded_token", facts_grounded, lambda x: 1
...
def generate_datasets2(buckets,
    ...
    for label, bucket_results in enumerate(buckets):
        ...
        logger.info("Number of samples: {}".format(num_samples))
        print(label, [(kind_name, feature_name,
                       feature_results.avg(), feature_results.std()) for kind_name, kind_results in
                      kinds_results.items()
                      for feature_name, feature_results in kind_results.items()])
        ...
        for i in range(num_samples):
            ...
            current_label_samples.append((candidate_example, label, id))
...
def train_detector(models, args):
    ...
    results, buckets = group_results2(read_json(buckets_paths[0]),
                                      read_json(buckets_paths[1]), args)
    # print([x[0] for x in results[0]])
